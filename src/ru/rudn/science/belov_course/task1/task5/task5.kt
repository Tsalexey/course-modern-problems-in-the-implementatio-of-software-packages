...
fun main(args: Array<String>) {
    ...
    (0..M - 1).forEach { m ->
        (0..N - 1).forEach { n ->
            ...
            u.getValue(m + 1).put(n + 1, (BigDecimal.valueOf(h) * u_np1_m + u_n_m * BigDecimal.valueOf(tau) * u_n_mp1) / (BigDecimal.valueOf(h) + u_n_m * BigDecimal.valueOf(tau)))
        }
    }
    ...
    for (t in listOf(0, 5, 10)) {
        ...
        File(file2).printWriter().use { out ->
            (0..M).forEach { m ->
                (0..N).forEach { n ->
                    if ((0 + tau * m) - t.toDouble() < 0.00001) out.println("${0 + h * n};${u.getValue(m).getValue(n)}")
                }
            }
        }
    }
